...
var state = {
    ...
    challengeRating: function () {
        var fraction = this.current.level * modifiers[this.current.modifier].experience;
        return Math.floor(fraction);
    },
    toAbilityScore: function (mod) {
        return 10 + Math.floor(mod * 2);
    },
    loadCreatureCompendium: function () {
        var data = JSON.parse(localStorage.getItem("compendium"));
        this.list = data || [];
    }
};
var SelectComponent = function (_a) {
    var name = _a.name, onchange = _a.onchange, choices = _a.choices, current = _a.current;
    return {
        view: function (el) {
            ...
            }, [map(function (choice) { return mithril("option", {
                    value: choice,
...
var SelectLevelComponent = function () {
    return SelectComponent({
        name: "level",
        onchange: function (lvl) {
            state.set({ level: parseInt(lvl) });
        },
        current: state.current.level.toString(),
        choices: map(function (i) { return i.toString(); }, range(-5, 36)),
    });
};
...
var StatBlock = {
    ...
    AbilitiesBlock: {
        view: function () {
            return mithril(".abilities-block", [
                this.format("str"),
                this.format("dex"),
                this.format("con"),
                this.format("int"),
        ...
        format: function (mod) {
            var score = state.current["ability_modifiers"][mod];
            return mithril(".score.".concat(mod), [
                mithril("span.stat", mod),
                mithril("span.score", [
    ...
    ChallengeRating: {
        view: function () {
            var cr = state.challengeRating();
            return mithril(".property-line.challenge", [
                mithril("b", "Challenge"), ": ",
    ...
    BaseProperties: {
        view: function () {
            return mithril(".base-properties", [
                mithril(".property-line.armor-class", [
                    mithril("b", "Armor Class"), ": ",
                    state.current["armor_class"],
                ]),
                mithril(".property-line.hit_points", [
                    mithril("b", "Hit Points"), ": ",
                    state.current["hit_points"],
                ]),
                mithril(".property-line.speed", [
                    mithril("b", "Speed"), ": ",
        ...
        view: function () {
            return mithril(".property-lines", [
                mithril(".property-line.damage-immunities", [
                    mithril("b", "Damage Immunities"),
                    ["poison", "psychic", "#todo"].join(", ")
                ]),
                mithril(".property-line.condition-immunities", [
                    mithril("b", "Condition Immunities"),
                ...
                mithril(StatBlock.ChallengeRating),
            ]);
        }
    },
    PropertyBlocks: {
    ...
    view: function () {
        return mithril(".stat-block", [
            ...
            mithril("div.stat-block", [
                ...
                mithril("hr"),
                mithril(this.BaseProperties),
                mithril("hr"),
                mithril(this.AbilitiesBlock),
                mithril("hr"),
                mithril(this.PropertyLines),
                mithril("hr"),
                mithril(this.PropertyBlocks),
                mithril(this.ActionsBlock),
            ]),
            mithril("code", mithril("pre", JSON.stringify(state.current, null, 2))),
        ]);
    },
};
var SimpleMonsterCompendium = {
    oninit: function () {
        state.loadCreatureCompendium();
    },
    view: function () {
        return mithril("div.monster-list", [
            mithril("h1", "List of creatures"),
            mithril("ul", map(function (creature) {
                mithril("li", [
                    mithril("span.name", creature.name),
                    mithril("span.level", creature.level),
                    mithril("span.role", creature.role),
                    mithril("span.modifier", creature.modifier),
                ]);
            }, state.list))
        ]);
    }
